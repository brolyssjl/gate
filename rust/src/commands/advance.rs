...
pub fn advance(root: &Path, run: &mut Run) -> Result<Advance, UserError> {
    ...
    if let Some(to) = next_phase(from, &run.profile) {
        run.phase = to;
        // Belt-and-suspenders: a phase is never re-entered within one run's
        // profile sequence, so `to` already has streak 0 - but a phase
        // advance is documented as an explicit reset trigger, so make it
