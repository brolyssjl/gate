...
/// (Howard Hinnant's `civil_from_days`; see `core/run.rs`, which has its own
/// copy for the same reason: no shared pub helper exists in the wave-2
/// module this file must not otherwise depend on).
