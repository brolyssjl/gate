...
import { headSha, makeRepo, writeFile } from "./helpers.js";
...
function runOn(root: string, phase: Run["phase"], baseRef: string | null): Run {
...
describe("PLAN gate", () => {
  it("passes a valid, approved plan", () => {
    ...
    const res = planGate({ root, run: runOn(root, "PLAN", null), config: EMPTY_CONFIG });
  ...
  it("fails when not approved", () => {
    const root = makeRepo();
    writePlan(root, GOOD_PLAN.replace("approved: true", "approved: false"));
    const res = planGate({ root, run: runOn(root, "PLAN", null), config: EMPTY_CONFIG });
...
describe("IMPLEMENT gate", () => {
  ...
  it("fails with no diff", () => {
    ...
    const res = implementGate({ root, run: runOn(root, "IMPLEMENT", base), config: EMPTY_CONFIG });
  ...
  it("ignores .gate/ bookkeeping when measuring the diff", () => {
    const { root, base } = setup();
    // Only a .gate change → still counts as no code diff.
    writeFile(root, ".gate/runs/r1/worklog.md", "note\n");
    const res = implementGate({ root, run: runOn(root, "IMPLEMENT", base), config: EMPTY_CONFIG });
    expect(check(res, "implement.diff")).toBe(false);
  });

  it("fails when the configured build command fails", () => {
    const { root, base } = setup();
    writeFile(root, "greet.js", "x\n");
    const config = { ...EMPTY_CONFIG, commands: { build: "exit 3" } };
    const res = implementGate({ root, run: runOn(root, "IMPLEMENT", base), config });
...
describe("TEST gate", () => {
  ...
  function setup(reportEmitCmd: string) {
    const root = makeRepo();
    writePlan(root, GOOD_PLAN);
    return { root, config: { ...EMPTY_CONFIG, commands: { test: reportEmitCmd } } };
  }
  ...
  it("passes a green suite with mapped criteria", () => {
    ...
    const res = testGate({ root, run: runOn(root, "TEST", null), config });
  ...
  it("fails when no test command is configured", () => {
    const root = makeRepo();
    writePlan(root, GOOD_PLAN);
    const res = testGate({ root, run: runOn(root, "TEST", null), config: EMPTY_CONFIG });
    expect(check(res, "test.command")).toBe(false);
  });
});
